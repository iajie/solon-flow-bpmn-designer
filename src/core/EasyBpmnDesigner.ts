...
import activitiModdleDescriptor from "../modules/descriptor/activitiDescriptor.json";
import flowableModdleDescriptor from "../modules/descriptor/flowableDescriptor.json";
...
export class EasyBpmnDesigner {
    ...
    protected innerBpmnModeler() {
        ...
        // 导入xml
        const xml = initModelerStr('easy_bpmn_process_1', '业务流程', this.options.prefix);
        this.bpmnModeler.importXML(value || xml).then(({ warnings }) => {
    ...
    protected onCreated() {
        ...
        const _header = this.container.querySelector(".easy-bpmn-designer-container-toolbar") || this.container;
        _header.appendChild(this.toolbar);

        const _main = this.container.querySelector(".easy-bpmn-designer-container-designer") || this.container;
        _main.appendChild(this.designer);

        const _footer = this.container.querySelector(".easy-bpmn-designer-container-panel") || this.container;
    ...
    private additionalModules() {
        ...
        if (Object.prototype.toString.call(this.options.additionalModel) === "[object Array]") {
            ...
            this.options.additionalModel && Modules.concat(this.options.additionalModel);
        }

        return Modules;
    }

    private moddleExtensions() {
        const Extensions: ModdleExtensions = {};
        // 插入用户自定义模块
        if (this.options.moddleExtension) {
            for (let key in this.options.moddleExtension) {
                Extensions[key] = this.options.moddleExtension[key];
            }
        }
        // 根据需要的 "流程类型" 设置 对应的解析文件
        if (this.options.prefix === "activiti") {
            Extensions.activiti = activitiModdleDescriptor;
        }
        if (this.options.prefix === "flowable") {
            Extensions.flowable = flowableModdleDescriptor;
        }
        if (this.options.prefix === "camunda") {
