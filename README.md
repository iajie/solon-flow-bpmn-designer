<h1 align="center"><a href="https://www.easy-flowable.online/component" target="_blank">Easy-Bpmn-Designer</a></h1>
<h4 align="center"><a href='https://gitee.com/iajie/easy-bpmn-designer/stargazers'><img src='https://gitee.com/iajie/easy-bpmn-designer/badge/star.svg?theme=gvp' alt='star' /></a></h4>

<h2 align="center">简易 开源流程设计器</h2>
<h3 align="center">开箱即用、支持所有前端框架</h4>


![](./docs/index-banner.png)

## 开始关注并使用 Easy-Bpmn-Designer

给我们 star，这样，在我们发布新的版本时，您可以及时获得通知。


## 什么是 Easy-Bpmn-Designer

Easy-Bpmn-Designer 是一个基于bpmn重新加工的流程设计器，它基于 Web Component，因此支持 Layui、Vue、React、Angular 等几乎任何前端框架。它适配了 PC Web
端~~和手机端~~，并提供了 亮色 和 暗色 两个主题。除此之外，它还提供了灵活的配置，开发者可以方便的使用其开发流程设计的应用。

更多关于 Easy-Bpmn-Designer，请访问官网： https://www.easy-flowable.online


## Easy-Bpmn-Designer 的与众不同之处

### 1、AI 驱动的编辑器
Easy-Bpmn-Designer 的目标是打造一个完全由 AI 驱动的富文本编辑器，同时支持对接任意大模型，包括私有的大模型。允许用户使用自己私有的大模型 ApiKey。
不仅如此，Easy-Bpmn-Designer 其所有的 AI 功能，都允许用户自定义自己的 prompt，以及扩展的 AI 菜单。


### 2、简单、友好、易用

Easy-Bpmn-Designer 基于 Web Component 开发，支持与任意主流的前端框架集成。Easy-Bpmn-Designer 使用更加友好的 LGPL 开源协议，通过 `npm i easy-bpmn-designer` 使用，
不用担心 GPL 协议可能带来的 GPL 传染问题。


## 在线演示
https://www.easy-flowable.online/component/designer

## 已完善

- [x] **基础**：网格、流程模拟、颜色设置、小地图导航、导入文件、下载文件、预览XML、预览JSON、撤销、重做、放大缩小、重置、
- [x] **属性面板**：基础属性、表单属性、条件属性、多实例配置、流转条件、超时设置、高级设置、审批人属性、按钮配置
- [x] **更多**：国际化、亮色主题、暗色主题...

## 开发中...

- [ ] 适配flowable、solon-flow、activi...
- [ ] 完善细枝末节


## 快速开始

访问官网：https://easy-flowable.online/component